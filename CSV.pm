...
sub open_table ($$$$$) {
    ...
    my $tables = $data->{Database}->{csv_tables};
    if (!exists($tables->{$table})) {
	$tables->{$table} = {};
    }
    my $meta = $tables->{$table};
    if (!exists($meta->{csv})) {
	if (!exists($meta->{csv_class})) {
	    $meta->{csv_class} = 'Text::CSV_XS';
	    require Text::CSV_XS;
	}
	my $class = $meta->{csv_class};
	$meta->{csv} = $class->new({ binary => 1,
				     eol => "\015\012" });
    }				     
	...
	$tbl->{csv} = $meta->{csv};
	if (!$createMode) {
	    my($array, $skipFirstRow);
	    if (exists($meta->{skip_first_row})) {
		$skipFirstRow = $meta->{skip_first_row};
	    } else {
		$skipFirstRow = !exists($meta->{col_names});
	    }
	    if ($skipFirstRow) {
		$array = $tbl->fetch_row($data);
		if (!$array) {

...
sub run_test
{
    ...
    do_ok ($dbh, "insert into $tbl values (1, 'Foo')",	"insert");
    ok ($sz < -s $tbl_file,				"file grew");
    $sz = -s $tbl_file;
    
    do_ok ($dbh, "delete from $tbl where id = 1",	"delete single");
    ...
    my  $zero_ret = do_ok ($dbh, "delete from $tbl where id = 99", "delete");
    ok ($zero_ret, "true non-existing delete RV (via do)");
    cmp_ok ($zero_ret, "==", 0,    "delete non-existing row (via do)");
    is (do_ok ($dbh, "delete from $tbl where id =  9", "delete one"),  1,    "delete single (count) (via do)");
    is (do_ok ($dbh, "delete from $tbl where id >  7", "delete two"), 2,    "delete more   (count) (via do)");
    ...
    is (execute_ok (prepare_ok ($dbh, "delete from $tbl where id =  7", "delete"), "execute"), 1, "delete single (count) (via prepare/execute)");
    is (execute_ok (prepare_ok ($dbh, "delete from $tbl where id >  4", "delete"), "execute"), 2, "delete more   (count) (via prepare/execute)");
